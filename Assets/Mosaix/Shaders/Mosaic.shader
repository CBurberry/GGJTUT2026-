...
#if SPHERE_MASKING
        float4x4 MaskMatrixArray[128];
        float sphereMaskArray[128];
        int MaskCount;
        float MaskSizeOuter;
...
#if SPHERE_MASKING
            {
                //Iterate over the sphere array
                float combinedMask = 0;
                for (int index = 0; index < MaskCount; index++) 
                {
                    // If MASKING is enabled, use the mask matrix.
                    float3 TransformedWorld = mul(MaskMatrixArray[index], float4(i.world, 1));
                    ...
                    sphereMaskArray[index] = clamp(sphere, 0, 1);
                    ...
                    {
                        combinedMask = sphereMaskArray[index];
                    }
                }
