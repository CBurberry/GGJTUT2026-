...
public class Mosaix: MonoBehaviour
{
    ...
    public List<GameObject> MaskingSpheres;
    ...
    {
        try {
            MaskingSpheres = FindObjectsByType<MaskingSphere>(FindObjectsInactive.Exclude, FindObjectsSortMode.None)
    ...
    private void OnPreRenderInner()
    {
        ...
        if (SphereMasking && MaskingSpheres.Count > 0)
        {
            ...
            Matrix4x4[] masks = new Matrix4x4[128];
            for (int i = 0; i< MaskingSpheres.Count; i++) 
            {
                Matrix4x4 mat = MaskingSpheres[i].transform.worldToLocalMatrix;
            ...
            Debug.Log("Masking Spheres: " + MaskingSpheres.Count);
            MosaicMaterial.SetMatrixArray("MaskMatrixArray", masks);
            MosaicMaterial.SetInt("MaskCount", MaskingSpheres.Count);
